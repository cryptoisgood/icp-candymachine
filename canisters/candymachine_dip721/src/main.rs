...
mod ledger {
    ...
    impl Ledger {
        ...
        pub fn owners_list(&self) -> HashSet<Principal> {
...
#[query(name = "uniqueHolders", manual_reply = true)]
#[candid_method(query, rename = "uniqueHolders")]
fn unique_holders() -> ManualReply<Result<HashSet<Principal>, NftError>>   {
    ledger::with(|ledger| ManualReply::one(&ledger.owners_list()))
}
...
fn owner_token_metadata(owner: Principal) -> ManualReply<Vec<TokenMetadata>> {
